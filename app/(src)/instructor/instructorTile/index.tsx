...
const InstructorTile = ({ data }: { data: TInstructorRelated }) => {
    ...
    return (<div className='card bg-transparent'>
        <div className='position-relative'>
            ...
            <Image unoptimized={true} width='500' height='500' src={data?.image} className='card-img' alt='course image' />

            <div className='card-img-overlay d-flex flex-column p-3'>
                <div className='w-100 mt-auto text-end'>

                    <Link href='#' className='badge text-bg-info rounded-1'><i className='fas fa-user-graduate me-2'></i>{data?.courses} </Link>
                    <Link href='#' className='badge text-bg-orange rounded-1'><i className='fas fa-clipboard-list me-2'></i>{data?.students} </Link>
                </div>
            </div>
        </div>
        ...
        <div className='card-body text-center'>
            ...
            <h5 className='card-title'><a href='#'>{data?.name}</a></h5>
