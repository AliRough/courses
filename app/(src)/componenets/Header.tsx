...
export default function Header({ children }: any) {
  ...
  const { data } = useGetUser(cookies.Authorization);

  console.log('useeeeeeeeeeeeeeeeeeeeer is', data);

  const [authUserdata, setAuthUser]: any = useRecoilState(authUserState);

  if (!authUserdata.name && data) {
  ...
  return (
    <>
      {/* Header START */}
      <header className='navbar-light tw-bg-white container max-sm:tw-px-0  sm:tw-rounded-3xl sm:tw-mt-4 tw-shadow-md tw-shadow-gray-500/25  '>
        {/* Nav START */}
        ...
        <nav className='navbar navbar-expand-xl nav-category border-0 pb-2'>
